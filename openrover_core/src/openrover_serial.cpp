...
using Cls = OpenRoverSerial;

OpenRoverSerial::OpenRoverSerial(std::string port) : Node("openrover_serial", "", true), motor_efforts_u8(MOTOR_EFFORT_HALT) {
	...
	sub_motor_efforts = this->create_subscription<msg::RawMotorCommand>("motor_speeds_commanded", std::bind(&Cls::on_motor_speed_commanded, this, _1), 1); //, writer_group);
	sub_raw_commands = this->create_subscription<msg::RawCommand>("raw_command", std::bind(&Cls::on_raw_command, this, _1), 16); // , writer_group);
...
std::vector<std::string> OpenRoverSerial::list_ftdi_ports()
{
...
void OpenRoverSerial::on_raw_command(openrover_core_msgs::msg::RawCommand::SharedPtr cmd)
{
...
void OpenRoverSerial::keepalive_callback() {
	msg::RawCommand cmd;
	cmd.verb = 10;
	cmd.arg = 40;
	auto efforts = motor_efforts_u8.load();
...
void OpenRoverSerial::read_callback()
{
...
void OpenRoverSerial::on_kill_motors()
{
	motor_efforts_u8 = MOTOR_EFFORT_HALT;
}


void OpenRoverSerial::on_motor_speed_commanded(openrover_core_msgs::msg::RawMotorCommand::SharedPtr msg)
{
