...
def generate_launch_description():
    ...
    declare_map_yaml_cmd = DeclareLaunchArgument(
        'map',
        default_value=os.path.join(rover_bringup_dir, 'maps', 'amazon_warehouse.yaml'),
    ...
    start_gazebo_server_cmd = ExecuteProcess(
        ...
        cwd=[rover_description_dir], output='screen')

    start_gazebo_client_cmd = ExecuteProcess(
        condition=IfCondition(PythonExpression([use_simulator, ' and not ', headless])),
        cmd=['gzclient'],
        cwd=[nav_launch_dir], output='screen')

    # urdf = os.path.join(nav_bringup_dir, 'urdf', 'turtlebot3_waffle.urdf')
    ...
    ekf_config = os.path.join(get_package_share_directory("rover_simulation"), 'config', 'ekf.yaml')
    robot_ekf = Node(
